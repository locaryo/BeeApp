...
<html lang="en">
	...
	<body style="font-family: sans-serif; box-sizing: border-box;">
		...
		<h1 style="text-align: center;">CONSTANCIA ESTUDIANTIL</h1>
		<?php foreach ($datos_e as $key): ?>
			
		<div style="text-align: justify; margin-top: 50px;">
			<p style="line-height: 30px; text-indent: 3em; text-align: left;">Quien suscribe Prof. Julio Medina, Director(a) de la Escuela Tecnica Robinsoniana Petroquimica y Agropecuaria "Alí Primera", ubicada en la parroquia Morón, Municipio Juan José Mora, Estado Carabobo, hace constar por medio de la presente que el (la) estudiante: <strong class="nombre"><?= $key->p_nombre_e?> <?= $key->s_nombre_e?> <?= $key->p_apellido_e?> <?= $key->s_apellido_e?></strong>, portador(a) de la C.I <strong><?= $key->ci_e?></strong>, cursa en este plantel el <strong>"<?= $key->curso?>"</strong> Año de la Mencion <strong>"<?= $key->mencion?>"</strong> en la Seccion <strong>"<?= $key->seccion?>"</strong>, en el año escolar: <?= date('Y')?>-<?= date('Y', strtotime($año.'+ 1 year'))?>.</p>
		</div>
		<?php endforeach ?>
